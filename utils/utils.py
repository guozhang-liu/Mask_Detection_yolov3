...
def rescale_boxes(boxes, current_dim, original_shape):  # 先缩放再减padding可能更简单
...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for c in tqdm.tqdm(unique_classes, desc="Computing AP"):
        ...
        if n_p == 0 and n_gt == 0:
            ...
            fpc = (1 - tp[i]).cumsum()  # fp为0矩阵对应位置与tp的0、1互换
            tpc = (tp[i]).cumsum()  # tp为 np.zeros(pred_boxes.shape[0])，当iou大于阈值时0矩阵的对应位置为1
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    nA = pred_boxes.size(1)  # 锚点数
    nC = pred_cls.size(-1)  # 类别数
    nG = pred_boxes.size(2)  # 网格特征图大小
