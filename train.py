...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_weights", type=str, default='checkpoints/yolov3_ckpt.pth', help="if specified starts from checkpoint model")
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_i, (_, imgs, targets) in enumerate(train_loader):
            ...
            train_loader.set_description(
                'epoch{}, {}/{}, loss: {}'.format(epoch, batch_i, len(dataloader), loss.float()))
            # ----------------
            #   Log progress
            # ----------------
        ...
        if epoch % opt.evaluation_interval == 0:
            ...
            if len(mAP) != 0:
                if AP.mean() >= mAP[-1]:
                    torch.save(model.state_dict(), "checkpoints/yolov3_ckpt_{:.2f}.pth".format(AP.mean()))
                    mAP.append(AP.mean())
                else:
                    continue
            else:
